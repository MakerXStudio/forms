...
import { useFormValidator } from '../components/ValidatedFormFactory';
...
function getDeepPropertyOfSchema(
  schema: z.ZodTypeAny,
  path: string[]
): z.ZodTypeAny {
  ...
  if (!isZodObject(schema)) {
    throw new Error(
      `Can't access property '${first}' of schema this is not an object`
    );
  }
  ...
  throw new Error(
    `Property '${first}' does not exist on schema: ${Object.keys(
      schema.shape
    ).join(', ')}`
  );
}
...
function isZodObject(
  schema: z.ZodTypeAny
): schema is z.ZodObject<Record<string, z.ZodTypeAny>, any, any> {
